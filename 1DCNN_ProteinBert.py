...
class CNNOD(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv1d(15599, 1024, kernel_size=7, stride=1, padding=3)
...
tain_file = open("path of train .txt file with labels and seqs", 'r').readlines()
train_seq_ids = []
train_seqs = []
train_lab = []
...
lab = []
emb = []
e1 = np.load(".npy path of train seqs embeddings", allow_pickle=True)
for i in range(len(e1)):
    emb.append(e1[i])
for i in range(30):
    lab.append((train_lab[i]))
features = np.array(emb)
...
print("============= TESTING ======================")
# # load test dataset
tain_file = open("path of test .txt file with labels and seqs", 'r').readlines()
seq_ids = []
seqs = []
lab = []
for line in tain_file:
...
features = np.load(".npy path of test seqs embeddings", allow_pickle=True)
...
with open(".txt file for storing test prediction", 'w') as f:
...
print("============= EVALUATION ======================")
# predicted file
ab_test = open(".txt file having stored test prediction", 'r').readlines()
predicts = []
for line in ab_test:
...
# groundtruth file
lab_file = open("path of test .txt file with labels and seqs", 'r').readlines()
label = []
